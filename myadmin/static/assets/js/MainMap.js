...
baseMap = sessionStorage.getItem("mapChange") === null ? baseMap : sessionStorage.getItem("mapChange");
...
function removeVectorLayer(prefix){
  ...
  map.getLayers().forEach(function(layer) {
    ...
    if (layer instanceof ol.layer.Vector && layer.get('name') && layer.get('name').startsWith(prefix)) {
...
function au2LayerRefresh() {
  $.ajax({
    url: `https://geoserver.erkhet-innovation.com/geoserver/wfs?service=wfs&version=2.0.0&request=GetFeature&typeName=satellite:au_level2&outputFormat=application/json${
      parseInt(globalAu2Code) !== 0 ? `&CQL_FILTER=code='${globalAu2Code}'` : ""
    }`,
    type: "GET",
...
function cadastre_area() {
  $.ajax({
    url: "https://geoserver.egazar.gov.mn/geoserver/geoware/wfs?service=wfs&version=2.0.0&request=GetFeature&typeName=geoware%3Aca_046_parcel_tbl&outputFormat=application/json",
    type: "GET",
    success: function (response) {
      if(cadastreArea)
      {
...
function tulburTable(au2){
  $.get("/myadmin/au2/tulbur/"+au2).then(e=>{ $($(".payCal_scroll")[0]).html(e)});
}
